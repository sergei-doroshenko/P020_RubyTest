...
class AboutRegularExpressions < Neo::Koan
  ...
  # Why?
  #
  # Because, they match as much as possible.
  ...
  # 
  # With ([...]) characters can be in any order. (|) defines an oredered sets.
  ...
  def test_sub_is_like_find_and_replace
    assert_equal "one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] } # replace by the first character of found group
  end

  def test_gsub_is_like_find_and_replace_all
    assert_equal "one t-t", "one two-three".gsub(/(t\w*)/) { $1[0, 1] } # replace by the first character of found group
  end
end
